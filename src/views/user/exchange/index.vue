      ...
      >
        <van-panel class="panel" v-for="(prize,index) in prizeList" :key="index">
          <div class="panel_header" slot="header">
            <van-cell :value="'开赛时间：'+prize.beginTime">
              ...
              <div class="panel_title" slot="title">
                <p>{{prize.nickname}}</p>
                <p>
                  <i>{{prize.match.name}}</i>
                  第{{prize.match.ranking}}名
                </p>
                <!-- <p>第{{prize.match.ranking}}名</p> -->
              </div>
            </van-cell>
          </div>
          <van-card :title="prize.prize.name" currency :thumb="prize.prize.img||goodsImgURL">
            <div class="prizetag" slot="tag">{{prize.prizeType}}</div>
            <div slot="desc">
              <p v-if="prize.prize.type == 0">{{prize.receivingDec}}</p>
              <p class="breakall" v-if="prize.prize.type == 1">
                <span>兑奖码：</span>
                {{prize.prize.pickupCode==''?'请等待用户到店提供兑奖码哦~':prize.prize.pickupCode}}
              </p>
            </div>
          </van-card>
          <div class="footer" slot="footer">
            ...
            <!-- 自提奖品只显示结果 -->
            <van-button
              v-if="prize.prize.type == 1||prize.prize.type == 3"
              class="btn outbtn"
              :class="{'gray':prize.receiving.status==3}"
              size="small"
            >{{prize.btnText}}</van-button>
            <!-- <van-button
            v-else
            class="btn"
            size="small"
    ...
    getPrizeList(data) {
      ...
      if (this.prizeList.length > 0) {
        this.prizeList.forEach(p => {
          // 开赛时间
          p.beginTime = timeFormate(
            p.match.beginTime * 1000,
            "YY/MM/DD HH:mm:ss"
          );
          //兑奖时间
